...
(function (root) {
  ...
  (function (namespace) {
    ...
    /**
     * reduce jQuery objects to a chunk of HTML & length
     * @param $element {jQuery}
     * @returns {String}
     */
    namespace.prettyPrintjQuery = function ($element) {
      var limit = 200;
      var output = jasmine.jQuery.elementToString($element);
    ...
    namespace.prettyPrint = function (val) {
      var output;
      ...
      default:
        if (val instanceof jQuery) {
          output = namespace.prettyPrintjQuery(val);
        } else {
          try {
    ...
    namespace.assertEventTriggeredWithData = function (actual, selector, expectedArg, fuzzyMatch) {
      ...
      result.message = function () {
        if (wasTriggered) {
          // was triggered, but data is incorrect
          var actualArg = jasmine.flight.events.eventArgs(selector, eventName, expectedArg)[1];
          return [
            'Expected event ' + eventName + ' to have been triggered with' + jasmine.flight.prettyPrint(expectedArg) + 'but was triggered with ' + jasmine.flight.prettyPrint(actualArg),
            'Expected event ' + eventName + ' not to have been triggered with' + jasmine.flight.prettyPrint(expectedArg)
          ];
        } else {
          // was not triggered
          return [
      ...
      toHaveBeenTriggeredOn: function () {
        return {
          compare: function (actual, selector) {
            ...
            result.message = function () {
              return [
                '<div class="value-mismatch">Expected event ' + eventName + ' to have been triggered on' + jasmine.flight.prettyPrint(selector),
                '<div class="value-mismatch">Expected event ' + eventName + ' not to have been triggered on' + jasmine.flight.prettyPrint(selector)
              ];
            }();
      ...
      toHaveCss: function () {
        return {
          compare: function (actual, prop, val) {
            ...
            actual = jasmine.flight.prettyPrint(actual);
            return result;
          }
        };
      }
